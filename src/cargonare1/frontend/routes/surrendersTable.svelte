<script>
    ...
    export async function getSurrenders() {
        ...
        if (res.ok) {
            ...
            if (res.status === 500 || res.status === 503) {
                window.alert(
                    "Aplicación/servidor no disponible en este momento"
                );
            }
        }
    ...
    async function getSurrendersP(limit, offset) {
        ...
        const res = await fetch(
            "/api/v2/surrenders-by-degrees-us/surrenders?limit=" +
                limit +
                "&offset=" +
                offset
        );
    ...
    async function loadSurrenders() {
        ...
        const res = await fetch(
            "/api/v2/surrenders-by-degrees-us/loadInitialData"
        );
    ...
    <Row>
        <Col>
            <InputGroupAddon addonType="prepend">
                ...
                <a href="#/surrenders/search"
                    ><button type="button">Buscar Abandono</button></a
                >
            </InputGroupAddon>
            ...
            <Input
                ...
                on:change={() => {
                    getSurrendersP(
                        document.getElementById("elementospag").value,
                        offset
                    );
                }}
            >
                <option value="" disabled selected>
                    Seleccione elementos por página</option
                >
        ...
        <tbody>
            <tr>
                <td
                    ><input
                        ...
                        bind:value={newSurrender.degree}
                    /></td
                >
                <td
                    ><input
                        placeholder="Ej: 2021"
                        bind:value={newSurrender.year}
                    /></td
                >
                <td
                    ><input
                        placeholder="Ej: 133"
                        bind:value={newSurrender.surrender_counts}
                    /></td
                >
                <td
                    ><input
                        placeholder="Ej: 267"
                        bind:value={newSurrender.new_students}
                    /></td
                >
                <td
                    ><input
                        placeholder="Ej: 5647.3"
                        bind:value={newSurrender.surrender_percent}
                    /></td
                >
                <td
                    ><input
                        placeholder="Ej: ETSII"
                        bind:value={newSurrender.center}
                    /></td
                >
            ...
            {#each surrenders as surrender}
                <tr>
                    <td
                        ><a
                            href="#/surrenders/{surrender.degree}/{surrender.year}"
                            >{surrender.degree}</a
                        ></td
                    >
